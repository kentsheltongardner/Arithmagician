...
class UndoData {
    ...
    public movers:      Array<Point>
    public playerCell:  Point
    public playerPorts: number
...
export default class Game {
    private _fpsTracker             = new FPSTracker(60)
    private _inputManager           = new InputManager()
    private _world                  = new World()
    private _camera                 = new Camera()
    private _playerCell             = new Point(0, 0)
    private _lastFrameTime          = 0
    private _movementFrame          = 0
    private _movementDirection      = new Vector(0, 0)
    private _leftDown               = false
    private _animationId            = 0
    private _soundsPlaying          = false
    private _reading                = false
    private _frame                  = 0

    private _undoStack              = new Array<UndoData>()

    private _flags                 = new Array<Uint8Array>(C.WorldWidthCells)
    private _darkness               = new Array<Uint8Array>(C.WorldWidthCells)
    private _concatenatedValues     = new Array<Array<string>>(C.WorldWidthCells)
    
    private _ambienceAudio          = new StreamingAudio('./music/entrance.mp3')
    private _laserAudio             = new BufferedAudio()
    private _zapAudio               = new BufferedAudio()
    private _footsteps              = new SoundSet()


    private _canvas:            HTMLCanvasElement
    private _cutoutCanvas:      HTMLCanvasElement
    private _context:           CanvasRenderingContext2D
    private _cutoutContext:     CanvasRenderingContext2D
...
// #######  #     #  #######     #     
    ...
    constructor() {
        ...
        this._cutoutCanvas      = <HTMLCanvasElement>document.getElementById('cutout-canvas')
        this._context           = <CanvasRenderingContext2D>this._canvas.getContext('2d')
        this._cutoutContext     = <CanvasRenderingContext2D>this._cutoutCanvas.getContext('2d')
        ...
        for (let i = 0; i < C.WorldWidthCells; i++) {
            ...
            this._concatenatedValues[i] = new Array<string>(C.WorldHeightCells)
    ...
    public async loadFromBlob(blob: Blob) {
    ...
    private calculateDarkness() {
        ...
        for (let i = 1; i < C.WorldWidthCells - 1; i++) {
            for (let j = 1; j < C.WorldHeightCells - 1; j++) {
                ...
                if (C.TypeIsImmobile[type]) this._darkness[i][j] = 1
            }
        }
...
// #######  #     #  #         #####      #     
    ...
    private mouseUpHandler(event: MouseEvent) {
        if (event.button === 0) this._leftDown = false
    }
    ...
    private async keyDownHandler(event: KeyboardEvent) {
        this._inputManager.keyDown(event)
    }
    private keyUpHandler(event: KeyboardEvent) {
        this._inputManager.keyUp(event)
    }
    private ignoreContextMenu(event: MouseEvent) {
...
//  #####   #        ######   #     #     #     #######  
    ...
    private grabVector(port: number) {
        switch (port) {
...
// #     #   #####      #     #######  #     #  #######  #     #     #     
    ...
    private clearFlags(flags: number) {
        for (let i = 0; i < C.WorldWidthCells; i++) {
            for (let j = 0; j < C.WorldHeightCells; j++) {
    ...
    private traversed(x: number, y: number)     { return this._flags[x][y] & C.FlagsTraversed   }
    private moving(x: number, y: number)        { return this._flags[x][y] & C.FlagsMoving      }
    private unlocked(x: number, y: number)      { return this._flags[x][y] & C.FlagsUnlocked    }
    private inactive(x: number, y: number)      { return this._flags[x][y] & C.FlagsInactive    }

    private traverse(x: number, y: number)      { this._flags[x][y] |= C.FlagsTraversed }
    private move(x: number, y: number)          { this._flags[x][y] |= C.FlagsMoving    }
    private unlock(x: number, y: number)        { this._flags[x][y] |= C.FlagsUnlocked  }
    private deactivate(x: number, y: number)    { this._flags[x][y] |= C.FlagsInactive  }

    private calculateMovements(x: number, y: number, dx: number, dy: number): boolean {
        if (this.traversed(x, y))       return true
        this.traverse(x, y)
        const type                      = this._world.objects[x][y]
        if (type === C.TypeEmpty)       return true
        if (C.TypeIsImmobile[type])     return false

        if (type === C.TypeGate && !this.unlocked(x, y)) return false

        this.move(x, y)

        const connections = this._world.objectConnections[x][y]
        if ((connections & C.BitsEast)  !== 0 && !this.calculateMovements(x + 1, y, dx, dy)) return false
        if ((connections & C.BitsSouth) !== 0 && !this.calculateMovements(x, y + 1, dx, dy)) return false
        if ((connections & C.BitsWest)  !== 0 && !this.calculateMovements(x - 1, y, dx, dy)) return false
        if ((connections & C.BitsNorth) !== 0 && !this.calculateMovements(x, y - 1, dx, dy)) return false

        const roofConnections = this._world.roofConnections[x][y]
        if ((roofConnections & C.BitsEast)  !== 0 && !this.calculateMovements(x + 1, y, dx, dy)) return false
        if ((roofConnections & C.BitsSouth) !== 0 && !this.calculateMovements(x, y + 1, dx, dy)) return false
        if ((roofConnections & C.BitsWest)  !== 0 && !this.calculateMovements(x - 1, y, dx, dy)) return false
        if ((roofConnections & C.BitsNorth) !== 0 && !this.calculateMovements(x, y - 1, dx, dy)) return false

        return this.calculateMovements(x + dx, y + dy, dx, dy)
    }

    private performMovements() {
        const objects           = this._world.objects
        ...
        this._undoStack.push(
            new UndoData(undoDirection, movers, this._playerCell.copy(), playerPorts, this._camera.currentFocus()))
        this._playerCell.x += this._movementDirection.dx
        this._playerCell.y += this._movementDirection.dy
    }

    private undoMovements() {
...
// #######  #     #   #####   #######  #     #   #####   
    ...
    private cellRect(minX: number, minY: number, maxX: number, maxY: number) {
    ...
    private lasers() {
        ...
        for (let i = 0; i < C.WorldWidthCells; i++) {
            for (let j = 0; j < C.WorldHeightCells; j++) {
                if (objects[i][j] !== C.TypeConstant) continue
    ...
    private deactivateVariable(cellX: number, cellY: number) {
        const objects           = this._world.objects
        const movementOffset    = this.movementOffset()
        ...
        for (let i = cells.left; i < cells.right; i++) {
            for (let j = cells.top; j < cells.bottom; j++) {
                if (objects[i][j] !== C.TypeDefinition) continue
                ...
                if (distanceSquared > C.DefinitionRadiusSquaredPixels) continue
    ...
    private fireLasers(
        ...
        lasers: Array<Laser>) {
            ...
            if ((ports & C.PortOutEast)     === C.PortOutEast)  this.fireLaser(x, y, mx, my, 1, 0,  value, valueType, lasers)
            if ((ports & C.PortOutSouth)    === C.PortOutSouth) this.fireLaser(x, y, mx, my, 0, 1,  value, valueType, lasers)
            if ((ports & C.PortOutWest)     === C.PortOutWest)  this.fireLaser(x, y, mx, my, -1, 0, value, valueType, lasers)
            if ((ports & C.PortOutNorth)    === C.PortOutNorth) this.fireLaser(x, y, mx, my, 0, -1, value, valueType, lasers)
    }
    private fireLaser(
        x: number, 
        y: number, 
        ...
        lasers: Array<Laser>) {
            ...
            if (perpendicular && laserMoving !== targetMoving) return
            ...
            let input1  = false
            if (dx === 1)       input1 = (ports & C.PortMaskWest)   === C.PortIn1West
            else if (dy === 1)  input1 = (ports & C.PortMaskNorth)  === C.PortIn1North
            else if (dx === -1) input1 = (ports & C.PortMaskEast)   === C.PortIn1East
            else if (dy === -1) input1 = (ports & C.PortMaskSouth)  === C.PortIn1South
            ...
            if (targetType === C.TypeDefinition && input1) {
                ...
                for (let i = cells.left; i < cells.right; i++) {
                    for (let j = cells.top; j < cells.bottom; j++) {
                        ...
                        if (objects[i][j] !== C.TypeVariable)   continue
                        // Don't propagate to variables with different letters
                        if (values[i][j] !== targetValue)       continue
                        // Don't propagate to inactive variables
                        if (this.inactive(i, j))                continue
            ...
            if (expectedOperandType !== C.OperandTypeAny && valueType !== expectedOperandType) return
            ...
            let input2 = false
            if (dx === 1)       input2 = (ports & C.PortMaskWest)   === C.PortIn2West
            else if (dy === 1)  input2 = (ports & C.PortMaskNorth)  === C.PortIn2North
            else if (dx === -1) input2 = (ports & C.PortMaskEast)   === C.PortIn2East
            else if (dy === -1) input2 = (ports & C.PortMaskSouth)  === C.PortIn2South
    ...
    private petrifyPlayer(lasers: Array<Laser>) {
        const objects = this._world.objects
    ...
    private unlockGates(lasers: Array<Laser>) {
        const objects = this._world.objects
        for (const laser of lasers) {
            const x             = laser.endCell.x
            const y             = laser.endCell.y
    ...
    private consistentLasers(lasers: Array<Laser>) {
...
// #     #  #######  #     #  ######   #######  #     #  
    ...
    private setLaserVolume() {
        ...
        for (let i = 0; i < C.WorldWidthCells; i++) {
            for (let j = 0; j < C.WorldWidthCells; j++) {
                ...
                if (type !== C.TypeConstant && type !== C.TypeVariable) continue
    ...
    private positionCanvas(canvas: HTMLCanvasElement, rect: Rect) {
    ...
    private renderGrab(renderOrigin: Point) {

    }

    private renderDarkness(renderOrigin: Point, renderCells: Rect) {
        ...
        for (let i = renderCells.x; i <= renderCells.right; i++) {
            for (let j = renderCells.y; j <= renderCells.bottom; j++) {
                ...
                let degree = 0
                if (se > degree) degree = se
                if (sw > degree) degree = sw
                if (nw > degree) degree = nw
                if (ne > degree) degree = ne
                ...
                this._context.drawImage(I.Darkness, offset, 0, C.CellSizePixels, C.CellSizePixels,
                    baseX, baseY, C.CellSizePixels, C.CellSizePixels)
            }
        }
    }

    private renderDefinitions(renderOrigin: Point) {
        const movementOffset        = this.movementOffset()
        const types                 = this._world.objects
        ...
        for (let i = 0; i < C.WorldWidthCells; i++) {
            for (let j = 0; j < C.WorldHeightCells; j++) {
                if (types[i][j] !== C.TypeDefinition) continue
    ...
    private renderShadows(renderOrigin: Point, renderCells: Rect) {
        const types             = this._world.objects
        const connections       = this._world.objectConnections
        ...
        for (let i = renderCells.x; i <= renderCells.right; i++) {
            for (let j = renderCells.y; j <= renderCells.bottom; j++) {
                ...
                const type = types[i][j]
                if (type === C.TypeEmpty || type === C.TypeOffLimits) continue
                ...
                for (let i = 1; i < 6; i++) {
                    this._cutoutContext.drawImage(tileset, 
                        offset * C.CellSizePixels, 0, C.CellSizePixels, C.CellSizePixels, 
                        baseX + i, baseY + i, C.CellSizePixels, C.CellSizePixels)
                }
            }
        }
    ...
    private renderWand(renderOrigin: Point) {
    ...
    private renderLight(renderOrigin: Point, playerPosition: Point) {
    ...
    private renderFloor(renderOrigin: Point, renderCells: Rect) {
                ...
                switch (C.TypeToTileset[type]) {
                    case C.TilesetConnected: {
                        this._context.drawImage(tileset, 
                            ...
                            x, y, C.CellSizePixels, C.CellSizePixels)
                        break
                    }
                    case C.TilesetBlock: {
    ...
    private renderLasers(renderOrigin: Point, lasers: Array<Laser>) {
    ...
    private renderLaserImpacts(renderOrigin: Point, lasers: Array<Laser>) {
        const scalar = (Math.sin(performance.now() / 1000 * C.Tau) + 1) / 2
        for (const laser of lasers) {
            let x0 = laser.startPixel.x - renderOrigin.x
            let y0 = laser.startPixel.y - renderOrigin.y
    ...
    private renderObjects(renderOrigin: Point, renderCells: Rect) {
        ...
        for (let i = renderCells.x; i <= renderCells.right; i++) {
            for (let j = renderCells.y; j <= renderCells.bottom; j++) {
                ...
                const value = values[i][j]
                if (value === C.EmptyString) continue
                ...
                if (value.length === 1) {
                    ...
                    this._context.drawImage(I.Font, 
                        ...
                        dstX, dstY, C.FontWidth, C.FontHeight)
                    continue
                }
    ...
    private renderRoof(renderOrigin: Point, renderCells: Rect) {
    ...
    private renderMessage() {
        ...
        for (let j = 0; j < h; j++) {
            ...
            for (let i = 0; i < w; i++) {
                ...
                this._context.drawImage(I.Font, offset, 0, C.FontWidth, C.FontHeight,
                    x, y, C.FontWidth, C.FontHeight)
            }
        }
    }
    private messageLines(message: string): Array<string> {
        let lineIndex   = 0
        let spaceIndex  = 0
    ...
    private static cellDisplaySize(): number {
    ...
    private static gameDisplayRect(): Rect {
        const size = Game.cellDisplaySize()
        const w: number = size * C.DisplayWidthCells
        const h: number = size * C.DisplayHeightCells
