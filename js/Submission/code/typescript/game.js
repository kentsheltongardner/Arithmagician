...
export default class Game {
    ...
    // #######  #     #  #######     #     
    constructor() {
        this._fpsTracker = new FPSTracker(60);
        this._inputManager = new InputManager();
        this._world = new World();
        this._camera = new Camera();
        this._playerCell = new Point(0, 0);
        this._lastFrameTime = 0;
        this._movementFrame = 0;
        this._movementDirection = new Vector(0, 0);
        this._leftDown = false;
        this._animationId = 0;
        this._soundsPlaying = false;
        this._reading = false;
        this._frame = 0;
        this._undoStack = new Array();
        this._flags = new Array(C.WorldWidthCells);
        this._darkness = new Array(C.WorldWidthCells);
        this._concatenatedValues = new Array(C.WorldWidthCells);
        this._ambienceAudio = new StreamingAudio('./music/entrance.mp3');
        this._laserAudio = new BufferedAudio();
        this._zapAudio = new BufferedAudio();
        this._footsteps = new SoundSet();
        ...
        if (this.isMovement()) {
            ...
            if (!direction.equals(Vector.Zero)) {
                ...
                if (this._inputManager.stayPressed()) {
                    ...
                    if (grab) {
                        move && (move = this.calculateMovements(grabCell.x, grabCell.y, dx, dy));
                    }
                    else {
                        if (direction.equals(Vector.East)) {
                            this._world.objectPorts[x][y] = C.PortOutEast;
