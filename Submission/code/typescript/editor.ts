...
import { CameraCopy, Copier, FloorCopy, ObjectCopy, RoofCopy } from './copier.js'
import Compressor from './compressor.js'

export default class Editor {
    private _scale                  = C.CellSizePixels
    private _modifierKey            = C.ModifierKeyNone
    private _mouseButton            = C.MouseButtonNone

    private _focus                  = new Point(C.WorldWidthCells / 2, C.WorldHeightCells / 2)
    private _mousePoint             = new Point(0, 0)
    private _gamePoint              = new Point(0, 0)
    private _inspectCell            = new Point(C.WorldWidthCells / 2, C.WorldHeightCells / 2)
    private _selectPoint            = new Point(0, 0)

    private _selection              = new Array(C.WorldWidthCells)
    private _selectionConnections   = new Array(C.WorldWidthCells)

    private _world                  = new World()
    private _copier                 = new Copier()
    private _memory                 = new Memory()

    private _broadcastChannel       = new BroadcastChannel('arithmagician')

    private _canvas:                HTMLCanvasElement
    private _context:               CanvasRenderingContext2D
    private _valueInput:            HTMLInputElement
    private _portInput:             HTMLInputElement
    private _toolSelect:            HTMLSelectElement
    ...
    constructor() {
        ...
        this._valueInput            = <HTMLInputElement>document.getElementById('value-input')
        this._portInput             = <HTMLInputElement>document.getElementById('port-input')
        this._toolSelect            = <HTMLSelectElement>document.getElementById('tool-select')
        ...
        const testButton = <HTMLElement>document.getElementById('test-button')
...
//  #####   #######  
    ...
    private buildIconSelected(selectedBuildIcon: HTMLElement) {
        const activeBuildGrid   = this.activeBuildGrid()
        const activeBuildIcons  = <NodeListOf<HTMLElement>>activeBuildGrid.querySelectorAll('.build-icon')
    ...
    private isLayerHidden(layer: number): boolean {
    ...
    private isLayerHighlighted(layer: number): boolean {
        const layerString   = C.LayerToString[layer]
        const selector      = `.highlight-checkbox[data-layer=${layerString}]`
        const checkbox      = <HTMLInputElement>document.querySelector(selector)
        return checkbox.checked
    }
    private isLayerLocked(layer: number): boolean {
        const layerString   = C.LayerToString[layer]
...
// #     #   #####    #####    #####   #######  
    ...
    private zoom(scalar: number, inverse: number) {
        this._scale *= scalar
    ...
    private pan(oldPoint: Point, newPoint: Point) {
...
// #     #  #######     #      #####   
    ...
    private keyDown(e: KeyboardEvent) {
        ...
        if (this._modifierKey === C.ModifierKeyCtrl) return
...
// ######    #####   #######  #######  ######   
    ...
    private build(type: number, point: Point, layer: number) {
        if (!C.InnerWorldRectCells.contains(point)) return
        const x = point.x
        const y = point.y
        switch (layer) {
            case C.LayerFloor: {
    ...
    private buildCell(type: number, x: number, y: number, types: Array<Uint8Array>, connections: Array<Uint8Array>) {
        if (types[x][y] !== type) this.eraseCell(x, y, types, connections)
    ...
    private connectNewCell(x: number, y: number, types: Array<Uint8Array>, connections: Array<Uint8Array>) {
...
// #######  #     #  #     #   #####   #######  
    ...
    private erase(point: Point, layer: Number) {
        const x = point.x
        const y = point.y
        if (!C.InnerWorldRectCells.contains(point)) return
    ...
    private eraseCell(x: number, y: number, types: Array<Uint8Array>, connections: Array<Uint8Array>) {
        const e = x + 1
        const s = y + 1
        const w = x - 1
        const n = y - 1
...
//  #####    #####   #     #  #     #  #######   #####      #     
    ...
    private connectCells(x: number, y: number, types: Array<Uint8Array>, connections: Array<Uint8Array>) {
...
// ######   #######   #####    #####    #####   #     #  #     #  #######   #####      #     
    ...
    private disconnectCells(x: number, y: number, connections: Array<Uint8Array>) {
        const gamePoint = this._gamePoint
        let cellX       = gamePoint.x - x
        let cellY       = gamePoint.y - y
...
//  #####   #######  #######  #######   #####      #     
    ...
    private beginDragSelect() {
        this._selectPoint = this._gamePoint.copy()
    }
    private beginDragUnselect() {
        this._selectPoint = this._gamePoint.copy()
    }
    private dragSelect() {
    ...
    private copyConnections(x: number, y: number, connections: number): number {
        const xe    = x + 1
        const ys    = y + 1
        const xw    = x - 1
        const yn    = y - 1
        ...
        if (!e)     off |= C.BitsEast   | C.BitsNortheast | C.BitsSoutheast
        if (!s)     off |= C.BitsSouth  | C.BitsSoutheast | C.BitsSouthwest
        if (!w)     off |= C.BitsWest   | C.BitsSouthwest | C.BitsNorthwest
        if (!n)     off |= C.BitsNorth  | C.BitsNorthwest | C.BitsNortheast
        if (!se)    off |= C.BitsSoutheast
        if (!sw)    off |= C.BitsSouthwest
        if (!nw)    off |= C.BitsNorthwest
        if (!ne)    off |= C.BitsNortheast
    ...
    private pasteSelection(copyString: string) {
        const data              = JSON.parse(copyString)

        const floorCopies       = <Array<FloorCopy>>data.floorCopies
        const objectCopies      = <Array<ObjectCopy>>data.objectCopies
        const roofCopies        = <Array<RoofCopy>>data.roofCopies
        const cameraCopies      = <Array<CameraCopy>>data.cameraCopies
        ...
        if (!this.isLayerLocked(C.LayerFloor)) {
            for (let floorCopy of floorCopies) {
                ...
                if (!C.InnerWorldRectCells.contains(new Point(floorCopy.x, floorCopy.y))) continue
                this.eraseCell(floorCopy.x, floorCopy.y, floor, floorConnections)
            }
            for (let floorCopy of floorCopies) {
                const x                 = floorCopy.x
                const y                 = floorCopy.y
                if (!C.InnerWorldRectCells.contains(new Point(x, y))) continue
        ...
        if (!this.isLayerLocked(C.LayerObject)) {
            for (let objectCopy of objectCopies) {
                ...
                if (!C.InnerWorldRectCells.contains(new Point(objectCopy.x, objectCopy.y))) continue
        ...
        if (!this.isLayerLocked(C.LayerRoof)) {
            for (let roofCopy of roofCopies) {
                ...
                if (!C.InnerWorldRectCells.contains(new Point(roofCopy.x, roofCopy.y))) continue
        ...
        if (!this.isLayerLocked(C.LayerCamera)) {
            for (let cameraCopy of cameraCopies) {
                ...
                if (!C.InnerWorldRectCells.contains(new Point(cameraCopy.x, cameraCopy.y))) continue
    ...
    private pasteConnections(connections: number, x: number, y: number): number {
        if (x === 1) {
...
// #     #  #######  #     #  ######   #######  #     #  
    ...
    private renderDragSelector(borderColor: string, interiorColor: string) {
        const selectRect            = Rect.fromPoints(this._selectPoint, this._gamePoint)
    ...
    private renderSimpleLayer(types: Array<Uint8Array>) {
        for (let i = 0; i < C.WorldWidthCells; i++) {
    ...
    private renderLayer(types: Array<Uint8Array>, connections: Array<Uint8Array>) {
        for (let i = 0; i < C.WorldWidthCells; i++) {
            for (let j = 0; j < C.WorldHeightCells; j++) {
                const type = types[i][j]
                if (type === C.TypeEmpty) continue
    ...
    private canvasSize(size: Size) {
        return new Size(this._scale * size.w, this._scale * size.h)
    }
    private canvasRect(gameRect: Rect) {
