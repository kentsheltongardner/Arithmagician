...
export default class Camera {
    ...
    static SPEED    = 0.125
    private _focus  = new Point(0, 0)


    public focus(p: Point, grid: Array<Uint8Array>) {
        // this._focus = p.copy()
        ...
        for (let i = 0; i < C.WorldWidthCells; i++) {
            for (let j = 0; j < C.WorldHeightCells; j++) {
                if (grid[i][j] !== C.TypeNoCamera) continue
    ...
    public track(p: Point, grid: Array<Uint8Array>) {
        const contacts = new Array<Point>()
        for (let i = 0; i < C.WorldWidthCells; i++) {
            for (let j = 0; j < C.WorldHeightCells; j++) {
                if (grid[i][j] !== C.TypeNoCamera) continue
                const adjustX = i * C.CellSizePixels + C.HalfCellSizePixels
        ...
        while (true) {
            ...
            for (const contact of contacts) {
                if (canX) {
                    if (Camera.overlap(nextX, this._focus.y, contact.x, contact.y)) canX = false
                }
                if (canY) {
                    if (Camera.overlap(this._focus.x, nextY, contact.x, contact.y)) canY = false
                }
            }
            if (!canX && !canY) break
            if (!canY) {
    ...
    public static overlap(x1: number, y1: number, x2: number, y2: number) {
        const dx            = x2 - x1
        const dy            = y2 - y1
